...
import numpy as np
from rbh_axial_asymptotic import asymptotic_series
...
def operator_series(model_name,q,order,mass=1.):
    f,fp,V=asymptotic_series(model_name,float(q),order+4,float(mass));n=min(len(f),len(fp),len(V))-1
    return conv(f,f,n),conv(f,fp,n),smconv(f,V,n),V

def mode_roots(model_name,q,order=30,mass=1.):
    *_,V=operator_series(model_name,q,order,mass);lead=np.real_if_close(V[4]).real
    vals,vecs=np.linalg.eigh(lead)
...
def dynamic_mode(model_name,q,exponent,vector,radial_order=22,omega_order=2,log_order=6,mass=1.):
    f2,ffp,fV,_=operator_series(model_name,q,radial_order+4*omega_order+4,mass)
    c=np.zeros((omega_order+1,radial_order+1,log_order+1,2),complex);c[0,0,0]=vector;V4=fV[4]
    for k in range(omega_order+1):
        ...
        for n in range(radial_order+1):
            ...
            _,s,vh=np.linalg.svd(A);tol=1e-11*max(1.,s[0]);null=vh.conj().T[:,s<tol]
            if null.shape[1]:
                ...
                if err>5e-8: raise RuntimeError(f'log order insufficient {model_name} k={k} n={n} e={e}: {err}')
            else: sol=np.linalg.solve(block,rhs)
            c[k,n]=sol.reshape(log_order+1,2)
    return c

def evaluate(c,e,omega,radii):
    rr=np.atleast_1d(np.asarray(radii,float));K,N,L,_=c.shape;val=np.zeros((len(rr),2),complex);der=np.zeros_like(val);w=omega**2
...
def build_modes(model_name,q,radial_order=22,omega_order=2,log_order=6,mass=1.):
    roots,_=mode_roots(model_name,q,radial_order+4*omega_order+4,mass)
    src=sorted(roots,key=lambda x:x[0])[:2];resp=sorted(roots,key=lambda x:x[0])[-2:]
    gsrc=max(src,key=lambda x:abs(x[1][0]));esrc=max(src,key=lambda x:abs(x[1][1]))
    gresp=max(resp,key=lambda x:abs(x[1][0]));eresp=max(resp,key=lambda x:abs(x[1][1]))
    return [(e,dynamic_mode(model_name,q,e,v,radial_order,omega_order,log_order,mass)) for e,v in (gsrc,esrc,gresp,eresp)]
