...
class C:
    spacing:float=.10;outer:float=180.;eps:float=1e-8;tf:float=150.;courant:float=.45;pulse:float=12.;width:float=3.;extract:float=25.
...
def fit(t,y,start=60,stop=120):
    ...
    for g in np.linspace(.25,.75,11):
        z=least_squares(res,[d[0],.09,g,0],bounds=([-2*scale,0,.15,-20],[2*scale,.45,.95,20]),max_nfev=4000);rr=np.linalg.norm(z.fun)/max(np.linalg.norm(d),1e-30)
...
def problem(model_name,ratio):
    model=master.build_model(model_name,ratio*master.extremal_charge(model_name,1.));return model.f,lambda r:master.potential_matrix(model,'axial',r,2),master.outer_horizon(model)

def schw():return lambda r:1-2/np.asarray(r),lambda r:master.schwarzschild_potential('axial',r,1.,2),2.

def peak_map():
    s=json.loads((ROOT/'results'/'hayward_fanwang_axial_broad_summary.json').read_text());return {(x['model'],float(x['ell_over_ell_ext'])):float(x['peak_omega_M']) for x in s}
...
def main():
    ...
    for (model_name,ratio),peak in sorted(peak_map().items()):
        t,y,rpk=evolve(*problem(model_name,ratio),C());f=fit(t,y);z=complex(f['frequency_M'],-f['damping_M']);sh=[];control_modes=[z]
        for name,val,c in variants:
            tv,yv,_=evolve(*problem(model_name,ratio),c);fv=fit(tv,yv);zz=complex(fv['frequency_M'],-fv['damping_M']);sh.append(abs(zz-z));control_modes.append(zz);conv.append(dict(model=model_name,ell_over_ell_ext=ratio,control=name,value=val,frequency_M=zz.real,damping_M=-zz.imag,shift=abs(zz-z),residual=fv['residual']))
        for st,sp in [(55,115),(65,125),(70,140)]:
            fv=fit(t,y,st,sp);zz=complex(fv['frequency_M'],-fv['damping_M']);sh.append(abs(zz-z));control_modes.append(zz);conv.append(dict(model=model_name,ell_over_ell_ext=ratio,control='fit_window',value=st,frequency_M=zz.real,damping_M=-zz.imag,shift=abs(zz-z),residual=fv['residual']))
        sep=abs(z.real-peak);detuning=sep/(-z.imag);robust=max(abs(peak-mode.real)/(-mode.imag) for mode in control_modes);aligned=bool(detuning<=1.);rows.append(dict(model=model_name,ell_over_ell_ext=ratio,real_axis_peak_M=peak,ringdown_frequency_M=z.real,ringdown_damping_M=-z.imag,quality_factor=z.real/(-2*z.imag),peak_ringdown_separation=sep,linewidth_detuning=detuning,robust_linewidth_detuning=robust,qnm_resonance_aligned=aligned,controls_preserve_alignment=bool((robust<=1.)==aligned),maximum_control_shift=max(sh),fit_residual=f['residual'],potential_peak_radius_M=rpk,schwarzschild_relative_error=serr))
    for fn,data in [('hayward_fanwang_axial_ringdown.csv',rows),('hayward_fanwang_axial_ringdown_convergence.csv',conv)]:
    ...
    (ROOT/'results'/'hayward_fanwang_axial_resonance_summary.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2))
